...
app.use("/assets", express.static(path.join(__dirname, "..", "public"), staticNoCacheOptions))
app.use("/uploads", express.static(uploadsDir, staticNoCacheOptions))
app.use(express.static(path.join(__dirname, "..", "vanilla", "public"), staticNoCacheOptions))
...
function isAppGeneratedListingId(id, prefix) {
  ...
  return new RegExp(`^${prefix}-\\d{13}-[a-z0-9]{6}$`).test(value)
}
...
app.get("/flat/:id", (_req, res) => {
  res.sendFile(path.join(__dirname, "..", "vanilla", "public", "flat", "index.html"))
})

app.get("/roommate/:id", (_req, res) => {
  res.sendFile(path.join(__dirname, "..", "vanilla", "public", "roommate", "index.html"))
})
...
app.use((req, res, next) => {
  ...
  const routeIndex = path.join(__dirname, "..", "vanilla", "public", req.path, "index.html")
  ...
  if (req.path === "/") {
    res.sendFile(path.join(__dirname, "..", "vanilla", "public", "index.html"))
    return
  }

  res.status(404).sendFile(path.join(__dirname, "..", "vanilla", "public", "404.html"))
})
